<div>
    <div class="row hide-on-med-and-up">
        <div class="col s12">
            <h3 class="center-align">Documentation</h3>
        </div>
    </div>
    <div class="row">
        <div class="col s12">
            <ul class="tabs">
                <li class="tab col s3"><a class="active" href="#cl">Check List</a></li>
                <li class="tab col s3"><a href="#assign">Assignment</a></li>
                <li class="tab col s3"><a href="#as">Application Design</a></li>
                <li class="tab col s3"><a href="#git">Git</a></li>
                <li class="tab col s3"><a href="#es">External Sources</a></li>
            </ul>
        </div>
        <div id="cl" class="col s12">
            <div class="card-panel">
                <span><iframe src="documentation/Requirements_Checklist.pdf#zoom=90"></iframe></span>
            </div>
        </div>
        <div id="assign" class="col s12">
            <div class="card-panel">
                <span><iframe src="documentation/COMP4513_Assignment_1(vers2).pdf#zoom=90"></iframe></span>
            </div>
        </div>
        <div id="as" class="col s12">
            <div class="card-panel">
                <span><iframe src="documentation/MVC_Structure_Explained.pdf#zoom=90"></iframe></span>
            </div>
        </div>
        <div id="git" class="col s12">
            <div class="card-panel">
                <span><iframe src="documentation/Git_Starter_Package.pdf#zoom=90"></iframe></span>
            </div>
        </div>
        <div id="es" class="col s12">
            <div class="card-panel">
                <table class="centered">
                    <tr>
                        <td>
                            <a href="http://materializecss.com/about.html">Material Design</a>
                        </td>
                        <td>
                            <a href="https://design.google.com/icons/">Google Material Icons and Fonts</a>
                        </td>
                        <td>
                            <a href="https://developers.google.com/chart/">Google Charts</a>
                        </td>
                        <td>
                            <a href="https://jquery.com/">JQuery</a>
                        </td>
                        <td>
                            <a href="http://materializecss.com/preloader.html">Materialize Loading Animation</a>
                        </td>
                        <td>
                            <!-- Dropdown Trigger -->
                            <a class='dropdown-button' data-hover="true" data-alignment="right" href='#' data-activates='dropdown1'>Images Used</a>

                            <!-- Dropdown Structure -->
                            <ul id='dropdown1' class='dropdown-content'>
                                <li><a href="https://commons.wikimedia.org/wiki/File:Cat_mosaic.JPG">Cat Mosaic</a></li>
                                <li><a href="https://www.gpsgiftgallery.com/index.php?main_page=index&cPath=199">Philadelphia Flyers Banner</a></li>
                                <li><a href="http://www.freakingnews.com/Donald-Trump-vs-Rosie-O-Donnell-Pictures----1290.asp">Rosie and Donald</a></li>
                                <li><a href="http://www.theprofitablefirm.com/blog/category/prospecting-leads/">Circular Analysis</a></li>
                                <li class="divider"></li>
                                <li><a href="http://duallygirl1748.deviantart.com/art/kitten-play-270925938">Read Me Logo</a></li>
                            </ul>
                        </td>
                    </tr>
                </table>
            </div>
        </div>
    </div>
</div>